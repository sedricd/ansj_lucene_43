...
/**
 * lucene EnglishAnalyzer 自带的英文词干分析器
 * 
 * @author sedric
 * 
 */
public class PorterStemmer {
	 ...
	 * FileInputStream(args[i]); byte[] buffer = new byte[1024]; int bufferLen,
	 * offset, ch;
	 * 
	 * bufferLen = in.read(buffer); offset = 0; s.reset();
	 * 
	 * while(true) { if (offset < bufferLen) ch = buffer[offset++]; else {
	 * bufferLen = in.read(buffer); offset = 0; if (bufferLen < 0) ch = -1; else
	 * ch = buffer[offset++]; }
	 * 
	 * if (Character.isLetter((char) ch)) { s.add(Character.toLowerCase((char)
	 * ch)); } else { s.stem(); System.out.print(s.toString()); s.reset(); if
	 * (ch < 0) break; else { System.out.print((char) ch); } } }
	 * 
	 * in.close(); } catch (IOException e) { System.out.println("error reading "
	 * + args[i]); } } }
	 */
}
